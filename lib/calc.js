...
module.exports = {
    combine: function (db, poll) {
        if (!config.supportSSR) return;
        var data = db.combine;
        var result = {};
        _.forEach(data, function (v, k) {
            result[k] = {};
            result[k].c_fsp = {};
            byPer(v, result[k].c_fsp);
        })
...
function byPer(data, obj) {


    obj.data = {};
    obj.count = data.length;
    obj.data[75] = percentile.calc(data, 75);


    /*
    var per = 0.75;
    var len = data.length;
    var perIndex =  Math.floor(len * per);
    var sortedData = data.sort(function (a, b) {
        return a - b;
    });
    var sortedData = v1.sort(function (v1, v2) {
        return v1 - v2;
    });
    var index;
